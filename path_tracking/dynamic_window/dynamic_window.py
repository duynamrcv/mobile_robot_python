...
class Config:
    def __init__(self, ob):
        ...
        self.max_yaw_rate = math.pi/4           # rad/s
        self.max_accel = 0.2                    # m/ss
        self.max_delta_yaw_rate = math.pi/4     # rad/s
        self.v_resolution = 0.01                # m/s
        self.yaw_rate_resolution = math.pi/180  # rad/s
        self.dt = 0.1                           # s
...
def predict_trajectory(x_init, v, y, config):
    ''' predict the trajectory with the input '''
    x = np.array(x_init)
...
def calc_control_and_trajectory(x, dw, config, goal, ob):
    ''' calculation final inut with dynamic window '''
    x_init = x[:]
    min_cost = float("inf")
    best_u = [0.0, 0.0]
...
def calc_to_goal_cost(trajectory, goal):
    ''' calculation to goal cost with angle difference '''
