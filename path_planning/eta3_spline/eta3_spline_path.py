...
class Eta3PathSegment(object):
    ...
    start_pose  - starting pose array  (x, y, theta)
    end_pose    - ending pose array (x, y, theta)
    eta         - shaping parameters
    kappa       - curvature parameters
    '''
    def __init__(self, start_pose, end_pose, eta=None, kappa=None):
        ...
        tmp3 = (5*eta[0]**2*kappa[0] + 2/3*eta[0]**3*kappa[1] + 2*eta[0]*eta[2]*kappa[0])*sa
        tmp4 = (15*eta[1] - 5/2*eta[3] + 1/6*eta[5])*cb
        tmp5 = (5/2*eta[1]**2*kappa[2] - 1/6*eta[1]**3*kappa[3] - 1/2*eta[1]*eta[3]*kappa[2])*sb
        self.coeffs[0, 4] = tmp1 - tmp2 + tmp3 - tmp4 - tmp5

        tmp1 = 35*(end_pose[1]-start_pose[1])
        tmp2 = (20*eta[0] + 5*eta[2] + 2/3*eta[4])*sa
        tmp3 = (5*eta[0]**2*kappa[0] + 2/3*eta[0]**3*kappa[1] + 2*eta[0]*eta[2]*kappa[0])*ca
        tmp4 = (15*eta[1] - 5/2*eta[3] + 1/6*eta[5])*sb
        tmp5 = (5/2*eta[1]**2*kappa[2] - 1/6*eta[1]**3*kappa[3] - 1/2*eta[1]*eta[3]*kappa[2])*cb
        self.coeffs[1, 4] = tmp1 - tmp2 - tmp3 - tmp4 + tmp5        

        # term u^5
        tmp1 = 84*(end_pose[0] - start_pose[0])
        tmp2 = (45*eta[0] + 10*eta[2] + eta[4])*ca
        tmp3 = (10*eta[0]**2*kappa[0] + eta[0]**3*kappa[1] + 3*eta[0]*eta[2]*kappa[0])*sa
        tmp4 = (39*eta[1] - 7*eta[3] + 1/2*eta[5])*cb
        tmp5 = (7*eta[1]**2*kappa[2] - 1/2*eta[1]**3*kappa[3] - 3/2*eta[1]*eta[3]*kappa[2])*sb
        self.coeffs[0, 5] = -tmp1 + tmp2 - tmp3 + tmp4 + tmp5

        tmp1 = 84*(end_pose[1] - start_pose[1])
        tmp2 = (45*eta[0] + 10*eta[2] + eta[4])*sa
        tmp3 = (10*eta[0]**2*kappa[0] + eta[0]**3*kappa[1] + 3*eta[0]*eta[2]*kappa[0])*ca
        tmp4 = (39*eta[1] - 7*eta[3] + 1/2*eta[5])*sb
        tmp5 = (7*eta[1]**2*kappa[2] - 1/2*eta[1]**3*kappa[3] - 3/2*eta[1]*eta[3]*kappa[2])*cb
        self.coeffs[1, 5] = -tmp1 + tmp2 + tmp3 + tmp4 - tmp5

        # term u^6
        tmp1 = 70*(end_pose[0] - start_pose[0])
        tmp2 = (36*eta[0] + 15/2*eta[2] + 2/3*eta[4])*ca
        tmp3 = (15/2*eta[0]**2*kappa[0] + 2/3*eta[0]**3*kappa[1] + 2*eta[0]*eta[2]*kappa[0])*sa
        tmp4 = (34*eta[1] - 13/2*eta[3] + 1/2*eta[5])*cb
        tmp5 = (13/2*eta[1]**2*kappa[2] - 1/2*eta[1]**3*kappa[3] - 3/2*eta[1]*eta[3]*kappa[2])*sb
        self.coeffs[0, 6] = tmp1 - tmp2 + tmp3 - tmp4 - tmp5

        tmp1 = 70*(end_pose[1] - start_pose[1])
        tmp2 = (36*eta[0] + 15/2*eta[2] + 2/3*eta[4])*sa
        tmp3 = (15/2*eta[0]**2*kappa[0] + 2/3*eta[0]**3*kappa[1] + 2*eta[0]*eta[2]*kappa[0])*ca
        tmp4 = (34*eta[1] - 13/2*eta[3] + 1/2*eta[5])*sb
        tmp5 = (13/2*eta[1]**2*kappa[2] - 1/2*eta[1]**3*kappa[3] - 3/2*eta[1]*eta[3]*kappa[2])*cb
        self.coeffs[1, 6] = tmp1 - tmp2 - tmp3 - tmp4 + tmp5

        # term u^7
        tmp1 = 20*(end_pose[0] - start_pose[0])
        tmp2 = (10*eta[0] + 2*eta[2] + 1/6*eta[4])*ca
        tmp3 = (2*eta[0]**2*kappa[0] + 1/6*eta[0]**3*kappa[1] + 1/2*eta[0]*eta[2]*kappa[0])*sa
        tmp4 = (10*eta[1] - 2*eta[3] + 1/6*eta[5])*cb
        tmp5 = (2*eta[1]**2*kappa[2] - 1/6*eta[1]**3*kappa[3] - 1/2*eta[1]*eta[3]*kappa[2])*sb
        self.coeffs[0, 7] = -tmp1 + tmp2 - tmp3 + tmp4 + tmp5

        tmp1 = 20*(end_pose[1] - start_pose[1])
        tmp2 = (10*eta[0] + 2*eta[2] + 1/6*eta[4])*sa
        tmp3 = (2*eta[0]**2*kappa[0] + 1/6*eta[0]**3*kappa[1] + 1/2*eta[0]*eta[2]*kappa[0])*ca
        tmp4 = (10*eta[1] - 2*eta[3] + 1/6*eta[5])*sb
        tmp5 = (2*eta[1]**2*kappa[2] - 1/6*eta[1]**3*kappa[3] - 1/2*eta[1]*eta[3]*kappa[2])*cb
    ...
    def calc_point(self, u):
        '''
        input: u - parametric representation of a point along the segment, 0 <= u <= 1
        returns: (x,y) of point along the segment
        '''
        assert(0 <= u <= 1)
    ...
    def calc_deriv(self, u, order=1):
        '''
        input: u - parametric representation of a point along the segment, 0 <= u <= 1
        returns: (d^nx/du^n, d^ny/du^n) of point along the segment, for 0 < n <= 2
        '''
        assert(0 <= u <= 1)
        assert(0 < order <= 2)
        if order == 1:
...
class Eta3Path(object):
    '''
    Eta3Path
    input: segments: a list of Eta3PathSegment instances defining a continuous path
    '''
    ...
    def calc_path_point(self, u):
        '''
        input: normalized interpolation point along path object, 0 <= u <= len(self.segments)
        returns: 2d (x,y) position vector
        '''
